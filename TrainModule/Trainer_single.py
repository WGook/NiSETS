...
class Trainer():
    ...
    def run(self):
        ...
        while self.step < self.train_num_steps:
            ...
            batch = [x.to(self.task.args.device, non_blocking=True) if type(x) == torch.Tensor else x for x in batch]
            ...
            if self.step != 0 and self.step % self.validation_step == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, batch in enumerate(self.valLoader):
                        ...
                        batch = [x.to(self.task.args.device) if type(x) == torch.Tensor else x for x in batch]
...
#==========================================================
    ...
    def load(self, log_id, milestone):
        ...
        model_states = torch.load(str(results_folder / f'task-{milestone}.pt'), map_location = 'cuda:{}'.format(self.task.args.device))
    ...
    # ================ model summary ===============
    def summary(self):
        self._log_device_info(self.task.args.device)
        self._model_info()
    ...
    def _log_device_info(self, device):
        ...
        _status = 'cpu' if device in ['cpu', 'CPU'] else self.task.args.device
